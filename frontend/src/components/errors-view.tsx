...
import type { LexicoError, SyntaxDiagnostic } from "@/lib/api";
import { ScrollArea } from "@/components/ui/scroll-area";
import { AlertCircle, Bot, ChevronRight, Lightbulb, Loader2, MapPin, Terminal } from "lucide-react";
...
type Filter = "todos" | "lexico" | "sintactico";
...
export function ErrorsView({ errores, syntaxDiagnostics, runtimeErrors, onClickError }: ErrorsViewProps) {
  const [filter, setFilter] = useState<Filter>("todos");
  const showLex = filter === "todos" || filter === "lexico";
  const showSyn = filter === "todos" || filter === "sintactico";
  const totalErrors = errores.length + syntaxDiagnostics.length + runtimeErrors.length;
          ...
          <div className="ml-auto flex rounded-md p-0.5" style={{ backgroundColor: "var(--color-surface-2)", border: "1px solid var(--color-border)" }}>
            {([
              ...
              <button
                ...
                style={{
                  backgroundColor: filter === value ? "var(--color-surface)" : "transparent",
                  color: filter === value ? "var(--color-text)" : "var(--color-muted)",
                }}
              >
                {label}
              </button>
